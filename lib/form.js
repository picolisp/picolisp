...
/*** Server-Sent Events ***/
function serverSentEvent(fun, id) {
   (new EventSource(SesId + "!" + fun)).onmessage = function(event) {
      var fld = document.getElementById(id);
      var val = event.data.substr(1);
      ...
      case "S":
         ...
         break;
      case "D":
         for (var n = parseInt(val); n > 0; --n) {
            var i = fld.innerHTML.indexOf("\n");
            if (i >= 0)
               fld.innerHTML = fld.innerHTML.substr(i + 1);
            else
               break;
         }
         break;
